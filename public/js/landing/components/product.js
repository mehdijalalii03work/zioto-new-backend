        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          <div class="flex flex-col items-center gap-4">
            <div class="relative w-full max-w-[300px]">
              <img id="main-product-image" src="${images[0]}" alt="${product.name}" class="w-full aspect-[517/800] object-cover rounded-2xl border border-zioto-gold/20">
              ${product.badge ? `<span class="absolute top-4 right-4 badge badge-gold text-base px-4 py-2">${product.badge}</span>` : ''}
            </div>
            ${images.length > 1 ? `
              <div class="flex gap-3 flex-wrap justify-center">
                ${images.map((img, i) => `
                  <button onclick="changeMainImage('${img}')" class="w-16 h-16 rounded-lg overflow-hidden border-2 ${i === 0 ? 'border-zioto-gold' : 'border-white/20'} hover:border-zioto-gold transition-colors">
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-6">
              <div class="flex items-center gap-3 p-3 bg-zioto-green-dark/30 rounded-lg">
              ...
              <div class="flex items-center gap-3 p-3 bg-zioto-green-dark/30 rounded-lg">
                ...
                <span class="text-sm text-white/70">ارسال رایگان</span>
              </div>
              <div class="flex items-center gap-3 p-3 bg-zioto-green-dark/30 rounded-lg">
              ...
              <div class="flex items-center gap-3 p-3 bg-zioto-green-dark/30 rounded-lg">
                ...
                <span class="text-sm text-white/70">پشتیبانی ۲۴ ساعته</span>
              </div>
            </div>
