...
<section class="hero-gradient relative overflow-hidden min-h-[70vh] flex items-center">
  <div class="absolute inset-0 overflow-hidden pointer-events-none">
    @for ($i = 0; $i < 20; $i++)
      <div class="particle" style="left:{{ rand(0, 100) }}%;top:{{ rand(0, 100) }}%;animation-delay:{{ round(rand(0, 30) / 10, 1) }}s"></div>
    @endfor
  </div>
...
<section id="products-section" class="container mx-auto px-4 py-16"
  ...
  <div class="flex justify-center gap-3 mb-10">
    <button @click="filter = 'all'"
            :class="filter === 'all' ? 'bg-zioto-gold text-[#111318]' : 'bg-white/10 text-white/70 hover:bg-white/15'"
            class="px-5 py-2 rounded-full text-sm font-medium transition-all">همه</button>
    <button @click="filter = 'gold'"
            :class="filter === 'gold' ? 'bg-zioto-gold text-[#111318]' : 'bg-white/10 text-white/70 hover:bg-white/15'"
            class="px-5 py-2 rounded-full text-sm font-medium transition-all">شمش طلا</button>
    <button @click="filter = 'silver'"
            :class="filter === 'silver' ? 'bg-zioto-gold text-[#111318]' : 'bg-white/10 text-white/70 hover:bg-white/15'"
            class="px-5 py-2 rounded-full text-sm font-medium transition-all">شمش نقره</button>
  </div>
  <div class="flex flex-wrap justify-center gap-6">
    @foreach ($products as $product)
      <div x-show="filter === 'all' || filter === '{{ $product['category'] }}'"
           class="product-card bg-[#1A1D23] rounded-2xl overflow-hidden cursor-pointer w-[325px] mx-auto"
           x-transition.duration.300ms>
        <a href="{{ route('landing.product', $product['slug']) }}" class="block">
          <div class="p-4 flex justify-center">
            <img src="{{ $product['image'] }}" alt="{{ $product['name'] }}" class="product-image w-[175px] aspect-[517/800] object-cover rounded-xl">
          </div>
          @if ($product['badge'])
            <span class="absolute top-3 right-3 badge badge-gold">{{ $product['badge'] }}</span>
          @endif
          <div class="px-5 pb-5">
            <div class="flex items-center gap-2 mb-2">
              <span class="text-xs px-2 py-1 rounded-full {{ $product['category'] === 'gold' ? 'bg-zioto-gold/20 text-zioto-gold' : 'bg-white/10 text-white/60' }}">عیار {{ $product['purity'] }}</span>
            </div>
            <h3 class="text-lg font-bold text-white mb-2">{{ $product['name'] }}</h3>
            <div class="flex items-center justify-between mb-3">
              <span class="text-zioto-gold font-bold text-xl">{{ number_format($product['price'] / 10) }} تومان</span>
            </div>
            <span class="block w-full btn-gold text-sm py-3 text-center rounded-lg">مشاهده و خرید</span>
          </div>
        </a>
      </div>
    @endforeach
